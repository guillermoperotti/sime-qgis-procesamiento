...
def procesar_capa(nombre_capa):
    ...
    fechas_rangos = [(fecha_inicio, primera_cota_superior)]
    ...
    for (fi, ff), color in zip(fechas_rangos, colores):
        regla = QgsRuleBasedRenderer.Rule(simbolo_base.clone())
        if ff:
            expresion = f'"#Fecha_Ultima_Habilitacion" >= \'{fi.strftime("%Y-%m-%d")}\' AND "#Fecha_Ultima_Habilitacion" < \'{ff.strftime("%Y-%m-%d")}\''
        ...
        etiqueta += " - HABILITADOS" if fi >= umbral_habilitados else " - VENCIDOS"
        regla.setLabel(etiqueta)
        regla.setFilterExpression(expresion)
        regla.symbol().setColor(QColor(color))
        raiz.appendChild(regla)
    ...
    # 🔁 Nuevo filtro: #Concepto = 16 y top 10 por #Capacidad
    capacidades = [
        (f.id(), f["#Capacidad"])
        for f in capa_hab_2020.getFeatures()
        if f["#Capacidad"] is not None and f["#Concepto"] == 16
    ]
    ...
    print(f"Se creó la capa '10MayoresHp {nombre_capa}' con Concepto=16.")
